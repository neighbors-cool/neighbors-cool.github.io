...
let win = false;
const asteroids = [];
const numberOfAsteroids = 6; // Reduced from 10
const minAsteroidSize = 4; // Reduced minimum size
const maxAsteroidSize = 8; // Reduced maximum size
const asteroidSizeMultiplier = 8; // Increased multiplier to make size differences more noticeable
...
class Ship {
  ...
  draw() {
    ...
    let newXL = this.pos.x + Math.cos(this.rads + Math.PI * 0.85) * 30;
    let newYL = this.pos.y + Math.sin(this.rads + Math.PI * 0.85) * 30;
...
class Bullet {
  ...
  checkForBoarderHit() {
    return this.pos.y - this.radius <= -150 || this.pos.y + this.radius >= canvas.height + 150 || this.pos.x + this.radius >= canvas.width + 150 || this.pos.x - this.radius <= -150;
  }
}
...
class Asteroid {
  ...
  reset(resetSize) {
    ...
    if (Math.random() < 0.5) {
      ...
      let offScreenY = Math.random() < 0.5 ? getRandomFloat(-150, -100) : getRandomFloat(canvas.height + 100, canvas.height + 150);
      ...
      let offScreenX = Math.random() < 0.5 ? getRandomFloat(-150, -100) : getRandomFloat(canvas.width + 100, canvas.width + 150);
    ...
    // create shape with vertices based on size
    this.shape = [];
    let vertices = 6 + this.size; // More vertices for larger asteroids
    for (let i = 0; i <= Math.PI * 2; i += (Math.PI * 2) / vertices) {
      this.shape.push(new Vector(this.pos.x + Math.cos(this.rads + i) * this.radius * Math.random(), this.pos.y + Math.sin(this.rads + i) * this.radius * Math.random()));
    }
  }

  checkForBulletHit(bullet) {
    return this.pos.y - this.radius <= bullet.pos.y && this.pos.y + this.radius >= bullet.pos.y && this.pos.x + this.radius >= bullet.pos.x && this.pos.x - this.radius <= bullet.pos.x;
  }
...
function moveEverything() {
  ...
  for (let i = 0; i < tempLength; i++) {
    ...
    if (a.checkForAsteroidHit()) {
      ...
      a.shape = [];
      let vertices = 6 + a.size;
      for (let j = 0; j <= Math.PI * 2; j += (Math.PI * 2) / vertices) {
        a.shape.push(new Vector(a.pos.x + Math.cos(a.rads + j) * a.radius * Math.random(), a.pos.y + Math.sin(a.rads + j) * a.radius * Math.random()));
      }

      let newA = a.deepCopy();
      newA.rads += Math.PI * 0.25;
      newA.vel = new Vector((Math.cos(newA.rads) * newA.speed) / framesPerSecond, (Math.sin(newA.rads) * newA.speed) / framesPerSecond);
      asteroids.push(newA);

      a.rads += Math.random() > 0.5 ? Math.PI / Math.random() : Math.PI * Math.random();
      a.vel = new Vector((Math.cos(a.rads) * a.speed) / framesPerSecond, (Math.sin(a.rads) * a.speed) / framesPerSecond);
    }
  }
...
function handleTouchMove(evt) {
  ...
  // Update ship rotation based on horizontal movement
  ship.rads += deltaX * 0.01;

  // Update ship speed based on vertical movement
  if (deltaY < 0 && ship.speed < maxShipSpeed) {
    ship.speed += 2;
  } else if (deltaY > 0 && ship.speed > 2) {
    ship.speed -= 2;
  }

  // Auto-fire while touching
  touchShootDelay--;
  if (touchShootDelay <= 0) {
    if (ship.bullets.length <= maxBulletCount) {
      ship.fire();
    }
    touchShootDelay = touchShootInterval;
  }
