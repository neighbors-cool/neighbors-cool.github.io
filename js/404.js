...
const pendulum = new TriplePendulum(canvas);
pendulum.init();
class TriplePendulum {
  ...
  updatePhysics() {
    ...
    const a1_acc =
      (-g * (2 * m1 + m2) * Math.sin(a1) - m2 * g * Math.sin(a1 - 2 * a2) - 2 * Math.sin(a1 - a2) * m2 * (v2 * v2 * l2 + v1 * v1 * l1 * Math.cos(a1 - a2))) /
    ...
    const a3_acc = (2 * Math.sin(a2 - a3) * (v2 * v2 * l2 * (m2 + m3) + g * (m2 + m3) * Math.cos(a2) + v3 * v3 * l3 * m3 * Math.cos(a2 - a3))) / (l3 * (2 * m2 + m3 - m3 * Math.cos(2 * a2 - 2 * a3)));
